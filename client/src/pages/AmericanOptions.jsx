...
import { FaChartLine, FaArrowLeft, FaBalanceScale, FaSignOutAlt, FaDollarSign, FaFlag } from 'react-icons/fa';
...
const AmericanOptions = () => {
  ...
  const handleCalculate = async () => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:5000/options/american/put', {
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <Link to="/european"><motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="flex items-center gap-2 px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium text-sm shadow-sm"><FaDollarSign size={14} /><span>European</span></motion.button></Link>
          ...
          {!showResults ? (
            ...
            >
              <h2 className="text-xl font-bold text-gray-900 mb-3 flex items-center gap-2">
                ...
                American Put Option Pricing (Binomial Tree)
              </h2>
              ...
              <div className="mb-6 bg-white rounded-lg shadow-sm border border-gray-100 p-6">
                <div className="flex items-center justify-center gap-3 mb-4">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600">American Put Option | Binomial Tree</p>
                  </div>
                </div>
                ...
                <div className="text-center p-6 bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl mb-4">
                  ...
                  <p className="text-4xl font-bold text-blue-600">${results.option_price.toFixed(4)}</p>
                </div>
              ...
              <div className="mb-6 bg-white rounded-lg shadow-sm border border-gray-100 p-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div className="bg-gradient-to-r from-orange-50 to-orange-100 p-4 rounded-xl">
                    ...
                    <p className="text-3xl font-bold text-orange-600">{results.early_optimal?.length || 0}</p>
                    <p className="text-xs text-gray-500 mt-1">out of {PERIOD_COUNTS[numPeriodsIdx]} periods</p>
                  </div>
                  <div className="bg-gradient-to-r from-blue-50 to-blue-100 p-4 rounded-xl">
                    <p className="text-xs text-gray-600 uppercase tracking-wide mb-1">Tree Size</p>
                    <p className="text-3xl font-bold text-blue-600">{results.stock_bt?.length || 0} x {results.stock_bt?.[0]?.length || 0}</p>
                    <p className="text-xs text-gray-500 mt-1">nodes in binomial tree</p>
                  </div>
                </div>
                {results.early_optimal?.length > 0 && (
                  <div className="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg text-sm text-orange-800">
                    <p className="font-medium mb-1">Early exercise is optimal at {results.early_optimal.length} node(s) in the tree.</p>
                    <p className="text-xs text-orange-600">
                      This means the American put is worth more than its European counterpart because of the early exercise premium.
                    </p>
                  </div>
                )}
                {results.early_optimal?.length === 0 && (
                  <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-800">
                    <p className="font-medium">No early exercise is optimal for this configuration.</p>
                    <p className="text-xs text-blue-600">
                      The American put has the same value as its European counterpart in this case.
                    </p>
                  </div>
                )}
              </div>
